...
NAN_METHOD(SocketWrap::BindToAddress) {
	...
	if (socket->family_ == AF_INET6) {
		...
		uv_ip6_addr(*Nan::Utf8String(info[0]), 0, &sin6_address);
#else
		String::Utf8String address (args[0]);
		sin6_address = uv_ip6_addr (*address, 0);
#endif
		rc = bind (socket->poll_fd_, (sockaddr *) &sin6_address, sizeof(sin6_address));
	} else {
#if UV_VERSION_MAJOR > 0
		uv_ip4_addr(*Nan::Utf8String(info[0]), 0, &sin_address);
#else
		String::Utf8String address (args[0]);
		sin_address = uv_ip4_addr (*address, 0);
#endif
		rc = bind (socket->poll_fd_, (sockaddr *)&sin_address, sizeof(sin_address));
	}
